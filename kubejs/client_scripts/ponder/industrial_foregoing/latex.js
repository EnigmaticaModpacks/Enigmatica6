...
onEvent('ponder.tag', (event) => {
  event.add('enigmatica6:industrial_foregoing', 'industrialforegoing:latex_processing_unit');
});

onEvent('ponder.registry', (event) => {
  event
    .create('enigmatica6:latex', [
    ...
    .scene(
      ...
      'enigmatica6:industrial_foregoing/latex/extracting_fluids',
      (scene, util) => {
        scene.showBasePlate();
        ...
        scene
          ...
          .text('Latex is a renewable resource gathered from trees, and plays a vital role in Industrial Foregoing.')
          .pointAt(util.vector().topOf(3, 4, 3));

        scene.idle(60);

        // hide basically everything except for the acacia log
        scene.world().hideSection(util.select().layersFrom(1).substract(util.select().position(4, 1, 4)), Facing.up);
        ...
        scene
          ...
          .text('Keep in mind that using different logs with the Fluid Extractor may give you different fluids!')
          .independent(60);
    ...
    .scene(
      ...
      'enigmatica6:industrial_foregoing/latex/processing_latex',
      (scene, util) => {
        scene.showBasePlate();
        ...
        scene
          ...
          .showText(60)
          .colored(PonderPalette.RED)
          .text(`This is quite a slow process; you'll have to wait around 5 seconds for each Tiny Dry Rubber...`)
          .placeNearTarget()
          .pointAt(util.vector().topOf(chestPos));
        ...
        scene.world().setBlock(util.grid().at(4, 1, 3), util.getDefaultState('minecraft:crafting_table'), false);
        ...
        scene
          .world()
          .setBlock(util.grid().at(4, 1, 5), Block.id('minecraft:furnace').with('facing', 'west').blockState, false);
        ...
        var procline = scene.world().showIndependentSection(util.select().fromTo(4, 1, 2, 4, 1, 6), Facing.down);
